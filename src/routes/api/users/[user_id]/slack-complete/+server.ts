...
export async function POST({ params, cookies }) {
    ...
    try {
        ...
        const response = await fetch(getBackendUrl(`/users/${userID}/slack-complete`), {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            return json({ error: 'Failed to complete slack link' }, { status: response.status });
        }
    } catch (error) {
        console.error('Error completing slack link:', error);
