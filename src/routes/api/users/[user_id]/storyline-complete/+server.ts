...
export async function POST({ params, cookies }) {
    ...
    try {
        ...
        const response = await fetch(getBackendUrl(`/users/${userID}/storyline-complete`), {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            return json({ error: 'Failed to complete storyline' }, { status: response.status });
        }
    } catch (error) {
        console.error('Error completing storyline:', error);
